...
Today, online social networks and other collaborative applications, like Facebook, Office Online, etc. are a hot topic in the world of computer science. Actually, these systems are inseparable from our daily lives. However, these systems are dealing with two major drawbacks. The biggest disadvantage is the fact that almost any of these applications requires an Internet connection in order to work. Moreover, with the rise of the mobile devices, along with their less reliable connection, the problem is getting reinforced even more. A less severe drawback can be found in privacy concerns, where the owner of the system can access all your personalized data. Both characteristics are due to the centralized notion of the systems. \\
\\
However, to solve these implications, the systems require a shift from a centralized view to a more distributed infrastructure. This allows the users to maintain their own data and work completely offline. Nonetheless, the complexity of the systems will grow strongly due to the necessity to replicate data over all the users, while inhibiting possible conflicts. \\
\\
To restrain developers with these complexities, S. Burckhardt introduced Cloud Types. In this preparatory, the model will be examined together with the concepts on which the model is based. This aims to clarify a specific implementation of Cloud Types, which is the topic of the next paper.
...
\chapter{State of The Art}\label{cha:StateOfTheArt} % 5
...
The goal of Cloud Types is to make eventually consistency programming more convenient by providing eventual consistent storage at the programming level. This is achieved by the abstraction of several implementation details in a uniform language, in order to hide these complexities for the developer. Concretely, the developer can declare data using particular types, called \textit{cloud types}, which are automatically shared between the different devices. By bringing eventual consistency to the programming level, the difficulty of writing possible applications strongly decreases. The model solves different challenges that are daunting to implement in offline available collaborative web applications. The most common challenges are summarized below.
...
\begin{figure}
    ...
    \small
    \centering
    \includegraphics[scale=0.5]{revision-diagram.pdf}
...
%http://en.wikipedia.org/wiki/Distributed_computing#Properties_of_distributed_systems
...
%http://link.springer.com/chapter/10.1007%2F978-3-662-43352-2_11#page-1
%http://research.microsoft.com/pubs/158085/ecr-esop2012.pdf